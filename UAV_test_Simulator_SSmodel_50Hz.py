...
class UAV_Attitude_Simulator():
    ...
    def reward(self):
        e_roll = abs(self.refmodel.refroll - self.uav.roll) + abs(self.refmodel.refrollrate - self.uav.rollrate) 
        self.r = - abs(e_roll)
    ...
    def observe(self):
        ...
        self.integr = self.integr + e_roll
        ob = np.array([e_roll, e_rollrate, self.integr])
